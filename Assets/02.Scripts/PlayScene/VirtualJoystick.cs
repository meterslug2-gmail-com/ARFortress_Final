...
public class VirtualJoystick : MonoBehaviour
{
    ...
    {
        if (DataManager.Instance.CheckControlable() == true)//내가 조종 가능상태인가?
        {
            ...
            if (PhotonManager.Instance.isMaster == true) //제어권이 있는 방장일때만 컨트롤러의 값을 동기화시킨다
            {
                ...
                if (PhotonManager.Instance.isFornt == false)//앞으로 못갈때
                {
                    //앞이 막혀 있을때?
                    if (JoyVec.y > 0)
                    {
                ...
                {
                    Front.renderF.material.color = Color.white;
                }

                if(PhotonManager.Instance.isBack==false)//뒤로 못갈때
                {
                    //뒤가 막혀 있을때
                    if(JoyVec.y<0)
                    {
                ...
                {
                    Back.renderB.material.color = Color.white;
                }
    ...
    public void DragEnd()
    {
        ...
        if (PhotonManager.Instance.isMaster==true&& DataManager.Instance.CheckControlable() == true)//제어권 있는 방장이면 스틱놓았을때 값을 동기화한다
        {
