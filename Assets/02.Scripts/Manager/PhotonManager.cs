...
public class PhotonManager : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    public int[] userArr = new int[4]; //유저 엑터 넘버를 저장
    ...
    public bool isFornt;//전진가능한가?
    public bool isBack;//후진 가능한가?
    public float hight;
    public bool isSingle;
    public int shooter;//지금 쏜사람이 누구냐
    private void Start()
    {
    ...
    public void Call_ChangeMasterClient()
    {
        if (isSingle == false)
        {
            //Debug.Log("턴 변환 시작");
            //제어권을 뺏는다
            //dataManager.ChangeControlable(false);//제어권 뻇고
            aliveCnt = dataManager.ComputeAliveUser(PhotonNetwork.PlayerList);
            //살아있는 유저수가 1보다 작으면 턴을전환할필요가 없으므로 리턴하고 끝낸다
            if (aliveCnt < 1)
            {
                Call_SnedAliveCnt(aliveCnt);//살아남은 사람 숫자가 1보다 작으면 게임 종료이므로 이사실은 다른사람한테도 알림
                return;
            }

            if (PhotonNetwork.IsMasterClient)//메소드 호출자가 마스터 클라이언트면? 방장만 이 것을 들어오게 하려는 것임.
            {
                Debug.Log("마스터 클라이언트 찾기 시작");
                ...
                {
                    masterIndex = 0;
                    Debug.Log("방장이 배열의 마지막이였음");
                    ChangeMasterClient(masterIndex);
                    //Debug.Log("플레이어 배열에서 마스터 클라이언트의 인덱스 위치 = "+ masterIndex);
                }
                else
                {
                    Debug.Log("방장이 배열의 마지막이 아니였음");
                    ChangeMasterClient(masterIndex + 1);
                    //Debug.Log("플레이어 배열에서 마스터 클라이언트의 인덱스 위치 = " + masterIndex +1);
                }
            }
        }
        else
        {
            //Debug.Log("혼자서 플레이 중이라 턴변환 없음");
            dataManager.ChangeControlable(true);
        }
    }
    /// <summary>
    ...
    /// <summary>
    /// 마스터 클라이언트가 바뀌면 호출되는 콜백
    /// </summary>
    /// <param name="newMasterClient"></param>
    public override void OnMasterClientSwitched(Photon.Realtime.Player newMasterClient)
    {
        //Call_ChangeTurn(newMasterClient.ActorNumber);
        nowTurn = newMasterClient.ActorNumber;//현재 턴을 넘겨 받은 엑터 넘버로 바꿔준다
        Debug.Log("현재 턴은 = " + nowTurn);
        if (nowTurn == myOrder)
        {
            dataManager.ResetmyTime();//플레이 가능한 남은 시간을 갱신한다
            //바뀐 턴이 내턴이다?
            Debug.Log("내턴이다");
            dataManager.ChangeControlable(true);//조종 가능하게
            //내체력이 0이하인지 검사한다 0이하면 플레이 불가능상태이므로 다시 턴을 넘겨주기 위해
            if (myHp <= 0)
            {
                dataManager.ChangeControlable(false);//조종 불가능하게
                Debug.Log("내턴이지만 체력이 0보자 작으니 턴을 다시 넘긴다");
                //턴을넘겨야지
    ...
    {
        //플레이어가 나가면 이플레이어를 게임 불가 상태로 바꿔줌
        if (SceneManager.GetActiveScene().buildIndex == 6)
        {
            dataManager.ChanegeAliveUserState(otherPlayer.ActorNumber, false);
            dataManager.DestroyUser(otherPlayer.ActorNumber);
        }
    }
    #endregion
    ...
    public void LeaveRoom()
    {
        ...
        {
            Call_DeleteMyInfoWaitingRoom(slotOrder);// 대기방에서 나가면 대기방에서 슬롯의 정보도 비워준다
        }
        //퇴장
    ...
    public void InstantePlayer()
    {
        ...
        //바닥 생성
        Instantiate(floor, hostingResultList[0].Result.Anchor.transform.position, Quaternion.identity,
            hostingResultList[0].Result.Anchor.transform);
    ...
    /// <param name="weapon"></param>
    [PunRPC]
    public void MakeMyRobot(int posNum, string leg, string body, string weapon)
    {
        
        GameObject player = Instantiate(robtObj, hostingResultList[posNum + 1].Result.Anchor.transform.position, Quaternion.identity,
            hostingResultList[posNum + 1].Result.Anchor.transform);//앵커의 자식으로 넣는다 ..
        ...
        player.GetComponent<Player>().actnum = PhotonNetwork.PlayerList[posNum].ActorNumber;
        Debug.Log((posNum + 1)+"플레이어 로봇 엑터 넘버" + PhotonNetwork.PlayerList[posNum].ActorNumber);
        //씬에 생성된 로봇들을 가지고 있는다
        dataManager.AddUser(PhotonNetwork.PlayerList[posNum].ActorNumber, player);
        dataManager.UserState(PhotonNetwork.PlayerList[posNum].ActorNumber, true);
    }
    #endregion
    ...
    public void Call_SendShootValue(bool check,float _lange,int actNum)
    {
        photonView.RPC("SendShootValue", RpcTarget.All, check,_lange,actNum);
    }
    [PunRPC]
    public void SendShootValue(bool check,float _lange,int actNum)
    {
        isFireCheck = check;
        lange = _lange;
        shooter = actNum;
    ...
    public void CheckDamage(int actNum, int damage)
    {
        if(actNum == myOrder)//내가 데미지 판정 대상이면?
        {
            ...
            if(myHp <=0)
            {
                //죽엇다고 알림 RCP로 던지는데 나의 엑터 넘버도 같이 날린다
                Call_MinusAliveCnt(myOrder);
            }
        }
    }
    #endregion
    ...
    public void Call_MinusAliveCnt(int dethActor)
    {
        photonView.RPC("MinusAliveCnt", RpcTarget.All, dethActor);
    }
    [PunRPC]
    public void MinusAliveCnt(int dethActor)
    {
        //aliveCnt -= 1;
        dataManager.ChanegeAliveUserState(dethActor, false);
        Debug.Log("죽은사람의 엑터넘버 = " + dethActor);
        //다른사람들한테 씬에서 내엑터넘버와 똑같은 로봇을 찾아서 더이상 충돌체크 안되게 콜라이더를 비활성화
        //내가 죽엇으면 호출되기때문에 데이터 매니저에 있는 로봇 리스트에서 내로봇은 콜라이더를 비활성화 시켜버린다
        dataManager.DestroyUser(dethActor);
    }
    #endregion

    #region 살아 있는 유저가 1명이면 다른사람들한테도 알려줌
    public void Call_SnedAliveCnt(int aliveCount)
    {
        photonView.RPC("SnedAliveCnt", RpcTarget.All, aliveCount);
    }
    [PunRPC]
    public void SnedAliveCnt(int aliveCount)
    {
        aliveCnt = aliveCount;
        Debug.Log("현재 방에 플레이 가능한 사람의 숫자는 = " + aliveCnt);
    }
    #endregion

    #region
    /// <summary>
    /// 마지막으로 죽어서 게임 게임 종료를 아는 사람이 다른 사람들에게도 얼라이브 카운트를 1이라고 알려줌
    /// </summary>
    public void Call_ChangeAliveCntOne()
    {
        photonView.RPC("ChangeAliveCntOne", RpcTarget.All);
    }
    [PunRPC]
    public void ChangeAliveCntOne()
    {
        aliveCnt = 1;
    }
    #endregion
